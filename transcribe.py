...
def transcribe_audio_from_url(
    ...
    try:
        ...
        # Extract a reasonable suffix from the URL path
        parsed_url = urlparse(audio_url)
        file_suffix = pathlib.Path(parsed_url.path).suffix or '.tmp' # Use .tmp if no extension

        with tempfile.NamedTemporaryFile(delete=False, suffix=file_suffix) as tmp_file:
            for chunk in response.iter_content(chunk_size=8192):
                tmp_file.write(chunk)
            temp_audio_file = tmp_file.name
        print(f"Audio downloaded temporarily to: {temp_audio_file}")
...
if __name__ == "__main__":
    ...
    if transcription_result:
        ...
        for i, segment in enumerate(word_segments[:5]):
            print(f"  {i+1}: Word='{segment.get('word', '')}', Start={segment.get('start', 'N/A'):.3f}, End={segment.get('end', 'N/A'):.3f}, Score={segment.get('score', 'N/A'):.3f}")
